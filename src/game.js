...
function syncSound(){
  ...
  document.querySelectorAll(".history-play").forEach(button=>{button.disabled=!speech.available||settings.muted||settings.speechVolume===0;});
  soundtrack.configure(settings);
}
...
function updateSpeechVolume(){
  settings=normalizeSettings({...settings,speechVolume:Number($('setting-speech-volume').value)/100});
  $('speech-volume-value').textContent=`${Math.round(settings.speechVolume*100)}%`;
  speech.stop();saveSettings();
}
$('setting-speech-volume').addEventListener('input',updateSpeechVolume);
$('setting-speech-volume').addEventListener('change',updateSpeechVolume);
$('overlay').addEventListener('click',event=>{
  const button=event.target.closest('.history-play');
  if(button&&(state==='paused'||state==='over')){
    const status=$('speech-status');status.textContent='';
    speech.speak(button.dataset.speak,button.dataset.lang,true,message=>{if(status.isConnected)status.textContent=message;});
  }
});
for(const id of ['setting-pronunciation','setting-sound','setting-music','setting-auto-music','setting-track','setting-mode','setting-transcription','setting-location']){
  $(id).onchange=()=>{
    soundtrack.unlock();
...
function togglePause(){if(state==='playing'){state='paused';soundtrack.setPlaying(false);modal('Передохнём?','Самса набирается сил.<br>Твой забег продолжится с этого места.','Продолжить ↗');}else if(state==='paused'){speech.stop();state='playing';soundtrack.setPlaying(true);document.activeElement?.blur();document.body.className='playing';}}
$('start').onclick=start;$('pause').onclick=togglePause;document.addEventListener('visibilitychange',()=>{if(document.hidden){speech.stop();if(state==='playing')togglePause();}});
