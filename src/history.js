...
const wordLink=(word,language)=>`<a class="history-word" href="https://forvo.com/word/${escape(encodeURIComponent(word))}/#${language}" target="_blank" rel="noopener noreferrer" title="Произношение на Forvo · откроется новая вкладка">${escape(word)}</a>`;
...
export function historyMarkup(entries,speechEnabled=true){
  const rows=entries.slice().reverse().map(e=>{
    ...
    const words=e.type==='word'?`<div class="history-pair"><div>${wordLink(e.target[source],source)}${play(e.target[source],languages[source].speech,speechEnabled)}${pronunciation(e.target[source+'IPA'],source)}</div><div>Правильно: <b>${wordLink(e.target[answer],answer)}</b>${play(e.target[answer],languages[answer].speech,speechEnabled)}${pronunciation(e.target[answer+'IPA'],answer)}</div></div>${!e.correct?`<div class="history-choice">Выбрано: <b>${wordLink(e.chosen[answer],answer)}</b>${play(e.chosen[answer],languages[answer].speech,speechEnabled)}${pronunciation(e.chosen[answer+'IPA'],answer)}</div>`:''}`:`<div>${e.type==='person'?'Встречный персонаж':'Препятствие на дорожке'}</div>`;
