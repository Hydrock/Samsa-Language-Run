...
export function historyMarkup(entries,speechEnabled=true){
  const rows=entries.slice().reverse().map(e=>{
    ...
    const words=e.type==='word'?`<div class="history-pair"><div>${escape(e.target.ru)}${play(e.target.ru,'ru-RU',speechEnabled)}${pronunciation(e.target.ruIPA)}</div><div>Правильно: <b>${escape(e.target.en)}</b>${play(e.target.en,'en-US',speechEnabled)}${pronunciation(e.target.enIPA)}</div></div>${!e.correct?`<div class="history-choice">Выбрано: <b>${escape(e.chosen.en)}</b>${play(e.chosen.en,'en-US',speechEnabled)}${pronunciation(e.chosen.enIPA)}</div>`:''}`:`<div>${e.type==='person'?'Встречный персонаж':'Препятствие на дорожке'}</div>`;
