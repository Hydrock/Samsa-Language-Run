...
export function historyMarkup(entries,speechEnabled=true){
  const rows=entries.slice().reverse().map(e=>{
    ...
    const words=e.type==='word'?`<div class="history-pair"><div>${escape(e.target[source])}${play(e.target[source],languages[source].speech,speechEnabled)}${pronunciation(e.target[source+'IPA'])}</div><div>Правильно: <b>${escape(e.target[answer])}</b>${play(e.target[answer],languages[answer].speech,speechEnabled)}${pronunciation(e.target[answer+'IPA'])}</div></div>${!e.correct?`<div class="history-choice">Выбрано: <b>${escape(e.chosen[answer])}</b>${play(e.chosen[answer],languages[answer].speech,speechEnabled)}${pronunciation(e.chosen[answer+'IPA'])}</div>`:''}`:`<div>${e.type==='person'?'Встречный персонаж':'Препятствие на дорожке'}</div>`;
